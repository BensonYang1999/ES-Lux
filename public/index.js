...
$(document).ready(function () {
    ...
    table_append()
    create_effect_table("./effect/OnMyOwn.json")
    music_select_append()
    effect_select_append()
    setInterval(timeupdate, 200);
    //setInterval(stat_update, 500);
    setInterval(new_stat_update, 500);
    //setInterval(checkbox_update, 500);
    //$("#select_effect").change(create_effect_table($('#select_effect :selected').text()));
    $("#select_effect").change(() => {
        $("#table3 td").remove();
        create_effect_table("./effect/" + $('#select_effect :selected').text())
        //var list = []
        //list.push($('#select_effect :selected').text())
        $.get("/change_effect?effect=" + $('#select_effect :selected').text());
        $("#effect-now-using").text("目前使用效果：" + $('#select_effect :selected').text())
    });
    $("#select_music").change(() => {
...
socket.on('connect_state', function (state) {
    for (var i = 0; i < NUM_OF_LUX; i++) {
        var id = "#" + (i + 1).toString() + "-state"
        var now = new Date();
        if (now - state[i] < 5000) {
            $(id).html("已連線")
            id = "#" + (i + 1).toString() + "-last"
            $(id).html("")
        }
        else {
            $(id).html("斷線")
            id = "#" + (i + 1).toString() + "-last"
            if (((now - state[i]) / 1000) >= 864000) $(id).html("從未連線")
            else $(id).html((now - state[i]) / 1000)
        }
    }
});
socket.on('effect_state', function (effect) {
    for (var i = 0; i < NUM_OF_LUX; i++) {
        var id = "#" + (i + 1).toString() + "-light"
        var lig = getEnumKey(ENUM_MODES, effect[i])
...
function new_stat_update() {
    socket.emit('connect_stat', (data) => { })
}
...
function effect_select_append() {
    var $effect_selection = $('#select_effect');
    $.get("/get_effect_list").success(function (data) {
        data.forEach(element => {
            $effect_selection.append($('<option>', {
                text: element
            }));
        });
    });
...
function create_effect_table(music) {
...
document.getElementById("music").ontimeupdate = function () {
    ...
    socket.emit('start', music_id.currentTime.toFixed(3) * 1000)
    $('#music-time').html(music_id.currentTime.toFixed(2))
    for (var i = 0; i < NUM_OF_EFFECT + 1; i++) {
        if (i == NUM_OF_EFFECT) {
            var time1 = "#effect-time-" + (i - 1).toString()
            var value1 = parseInt($(time1).text())
            if (value1 < (music_id.currentTime.toFixed(3) * 1000)) {
